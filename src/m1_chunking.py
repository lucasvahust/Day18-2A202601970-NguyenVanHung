...
def load_documents(data_dir: str = DATA_DIR) -> list[dict]:
    ...
    for fp in sorted(glob.glob(os.path.join(data_dir, "*.pdf"))):
        ...
        if text:
            ...
            print(f"  [Skip] Bo qua {os.path.basename(fp)}: PDF scan anh, khong co text layer (can OCR).")
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    # Chia từng parent thành các child chunks
    for parent in parents:
        pid = parent.metadata.get("parent_id")
        p_text = parent.text
        # Tách sentences hoặc paragraphs nhỏ
        parts = [p.strip() for p in p_text.split("\n\n") if p.strip()]
        if len(parts) <= 1:
            parts = [s.strip() for s in re.split(r'(?<=[.!?])\s+', p_text) if s.strip()] or [p_text]

        curr_child = ""
        for part in parts:
            if len(curr_child) + len(part) > child_size and curr_child:
